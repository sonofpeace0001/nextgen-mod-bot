...
def _is_immune(member) -> bool:
    """Check if member has an immune role (Elite, Admin, Escalation team)."""
...
async def _handle_phishing(bot, message):
    ...
    if total >= config.WARN_BEFORE_BAN or _is_obvious_bot(m):
        await _ban(bot, g, m, "Phishing")
    else:
...
async def _handle_spam(bot, message):
    ...
    if total >= config.WARN_BEFORE_BAN:
        await _ban(bot, g, m, "Repeated spam")
    elif total >= config.WARN_BEFORE_MUTE:
        await _timeout(bot, g, m, "Spam")
    else:
...
async def _handle_violation(bot, message, result):
    ...
    if sev >= 3:
        ...
        db.log_action(g.id, "BAN(severe)", m.id, "AutoMod", reason)
        await _ban(bot, g, m, reason)
        return
    if sev == 2:
        try: await message.delete()
        except: pass
        db.add_warning(g.id, m.id, "AutoMod", reason)
        db.log_action(g.id, "TIMEOUT(violation)", m.id, "AutoMod", reason)
    ...
    if total >= config.WARN_BEFORE_BAN:
        await _ban(bot, g, m, "Exceeded warning limit")
    elif total >= config.WARN_BEFORE_MUTE:
        await _timeout(bot, g, m, reason)
...
async def _timeout(bot, guild, member, reason):
    """Use Discord's built-in timeout instead of a muted role."""
    ...
    try:
        t = await llm.generate(
            ...
            f"Reason: {reason}. Be calm and direct."
        )
        await member.send(t)
    except: pass
